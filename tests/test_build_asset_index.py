...
class TestBuildAssetIndex(unittest.TestCase):
    ...
    def test_includes_manifest_command_for_run(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        script = repo_root / "scripts" / "build_asset_index.py"
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            figures_dir = paper_dir / "figures"
            out_path = paper_dir / "ASSET_INDEX.md"
            sweeps_dir = base / "doc" / "thesis_sweeps" / "session_a"

            run_id = "2026-03-06_000000_feedface"
            run_dir = runs_dir / run_id
            artifacts = run_dir / "artifacts"
            artifacts.mkdir(parents=True, exist_ok=True)
            sweeps_dir.mkdir(parents=True, exist_ok=True)

            payload = {"run_id": run_id, "phase": "02-kan-training", "kind": "kan"}
            (run_dir / "payload.json").write_text(json.dumps(payload))
            (artifacts / "eval_test.json").write_text(json.dumps({"rmse": 1.0, "mae": 1.0, "r2": 0.0}))
            (sweeps_dir / "manifest.json").write_text(
                json.dumps(
                    {
                        "planned": [
                            {
                                "run_id": run_id,
                                "cmd": ["modal", "run", "modal_jobs/kan_train.py", "--run-id", run_id],
                            }
                        ]
                    }
                )
